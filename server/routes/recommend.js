...
const fetchWithRetry = async (config, retries = 3) => {
    try { return await tmdb(config); } catch (error) {
        if (retries > 0) { console.log(`Retrying TMDB... (${retries})`); return fetchWithRetry(config, retries - 1); }
        throw error;
    }
};
    ...
    try {
      const completion = await groq.chat.completions.create({
          ...
          messages: [
              { role: "system", content: `You are a highly engaging, witty, and expert Movie Recommendation Assistant named 'Movie-Bhai'.
                Your personality:
                1. Talk like a cool friend using 'Hinglish' (a mix of Hindi and English).
                2. Never give short answers. If you find movies, explain WHY you're suggesting them in a fun way.
                3. Use emojis to make the chat lively. 🎬🔥😎
                4. For each movie you find, create a "why" key with a 1-line reason to watch it.
                Your task:
                1. Analyze the user's request.
                2. Formulate a fun, conversational Hinglish response.
                3. At the VERY END of your response, you MUST provide a single, clean JSON block containing the TMDB search parameters.
                Allowed JSON keys: genres (array), keywords (array), year (number), language ('hi' or 'en').

                Example:
                User asks: "i want a mind bending sci-fi movie"
                Your response:
                "Arre wah! Mind-bending sci-fi, matlab full dimaag ka dahi! 🧠 Here are a few top-class options I found for you. Inception to dekhi hi hogi, baap movie hai! Chalo, kuch aur batata hoon.
                {"genres":["sci-fi"],"keywords":["mind-bending", "twist ending", "dystopian"]}"
              `},
      ...
      if (jsonMatch) {
          try {
              ...
              console.error("Failed to parse JSON from AI response, using full text as query.");
          }
      }
      console.log("Conversational Text:", conversationalText);
      console.log("Parsed Query:", parsedQuery);
      
      let params = { api_key: process.env.TMDB_API_KEY, sort_by: "popularity.desc", "vote_count.gte": 100, include_adult: false, page: page };
      ...
      if (Array.isArray(parsedQuery.genres)) {
        const genreIds = parsedQuery.genres.flatMap((g) => {
          ...
          if (val.includes("romantic")) return [10749]; if (val.includes("comedy")) return [35];
          if (val.includes("action")) return [28]; if (val.includes("thriller")) return [53];
      ...
      const tmdbResponse = await fetchWithRetry({ method: "GET", url: "/discover/movie", params });
      const { results, page: currentPage, total_pages: totalPages } = tmdbResponse.data;

      if (!results || results.length === 0) {
        return res.json({ success: true, movies: [], has_more: false, message: "No movies found" });
      }
      ...
      const movies = filteredResults.map((movie) => ({
        ...
        why: movie.overview.substring(0, 100) + '...' // A simple "why" based on overview
      }));
      
      const hasMore = currentPage < totalPages;
      return res.json({ success: true, responseText: conversationalText, movies, has_more: hasMore });
    } catch (error) {
